...
const StaffOverlay: React.FC<StaffOverlayProps> = ({
  width,
  cellHeight,
  pitches,
  offsetY = 0
  ...
  const staffPitchNames = ["F5", "D5", "B4", "G4", "E4"];
  const staffYPositions = staffPitchNames.map(pitch => {
    const pitchIndex = pitches.indexOf(pitch);
    return offsetY + pitchIndex * cellHeight + cellHeight / 2;
  });

  const clefTop = offsetY + pitches.indexOf("A5") * cellHeight;
  ...
  return (
    <svg
      width={width}
      height={offsetY + pitches.length * cellHeight+20}
      style={{ position: "absolute", top: 0, left: 0, zIndex: 2, pointerEvents: "none" }}
    >
      {/* Staff lines */}
      {staffYPositions.map((y, i) => (
        <line
          key={i}
          x1={0}
          x2={width}
      ...
      {/* Treble clef path */}
      <g transform={`translate(-70, ${clefTop}) scale(1.2)`}>
        <path
          fill="black"
          d="M156.716,61.478c-4.111,6.276-8.881,11.511-14.212,15.609l-8.728,6.962c-13.339,11.855-22.937,21.433-28.542,28.464
        ...
        <path
          fill="black"
          d="M164.55,209.161c5.728-2.568,10.621-6.478,14.576-11.651c5.055-6.561,7.897-14.316,8.467-23.047
